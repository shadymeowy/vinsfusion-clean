...
NetVLADDB::NetVLADDB(const std::string &model_path, int history_size)
  ...
  if (num_inputs != 3) {
    throw std::runtime_error(
        "NetVLADDB: model should have exactly 3 inputs (image, descs, "
        "max_idx).");
  }
  if (num_outputs != 3) {
    throw std::runtime_error(
...
void NetVLADDB::query(const cv::Mat &image, std::vector<int> &best_idx,
  ...
  // If it is grayscale, convert to BGR
  if (image.empty()) {
    throw std::runtime_error("NetVLADDB: input image is empty.");
  }
  ...
  if (image_rgb.rows != 224 || image_rgb.cols != 224) {
    cv::resize(image_rgb, resized_image, cv::Size(224, 224), cv::INTER_LINEAR);
  } else {
    // Use original image if already 224x224
    resized_image = image_rgb;
  }
  cv::Mat hwc_u8 =
      resized_image.isContinuous() ? resized_image : resized_image.clone();
  ...
  // max_idx tensor (shape [0]), used to store the max index of the
  // descriptors in the cyclic buffer, it is used by model
  std::vector<int64_t> max_idx_vector{static_cast<int64_t>(max_idx)};
  std::vector<int64_t> max_idx_shape{1};
  ...
  Ort::Value max_idx_tensor = Ort::Value::CreateTensor<int64_t>(
      mem_info_cpu_, max_idx_vector.data(), max_idx_vector.size(),
      max_idx_shape.data(), max_idx_shape.size());
  ...
  auto outputs =
      session_.Run(Ort::RunOptions{nullptr}, input_name_ptrs.data(),
                   std::array<Ort::Value, 3>{std::move(image_tensor),
                                             std::move(descs_tensor),
                                             std::move(max_idx_tensor)}
                       .data(),
                   3, output_name_ptrs.data(), output_name_ptrs.size());

  if (outputs.size() != 3) {
    throw std::runtime_error("NetVLADDB: unexpected number of outputs.");
  }
