...
class TrackerTAPNext:
    def __init__(
        ...
        self.fast.setNonmaxSuppression(False)
    ...
    def track_image(self, frame_dist, max_cnt=None):
        ...
        if self.last_reset >= 25:
            print("Resetting model with new query points...")
            self.reset(frame)
        ...
        # If a track is previously invalid, and now valid, reset its count and assign a new ID
        # self.cnt[~self.is_valid] = 0
        # self.ids[~self.is_valid] = np.arange(
        #     global_id_counter,
        #     global_id_counter + np.sum(~self.is_valid),
        #     dtype=np.int32,
        # )
        # global_id_counter += np.sum(~self.is_valid)
...
class Tracker:
    def __init__(self, *args, **kwargs):
        self.tracker1 = TrackerTAPNext(*args, **kwargs)
        self.tracker2 = TrackerTAPNext(*args, **kwargs)
        self.tracker3 = TrackerKLT(*args, **kwargs)
        self.counter = 0
    ...
    def track_image(self, frame_dist, max_cnt=None):
        ...
        print(
            f"Tracker 1: {len(x1)} tracks, Tracker 2: {len(x2)} tracks, Tracker 3: {len(x3)} tracks"
        )
