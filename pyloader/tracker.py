...
class TrackerTAPNext:
    def __init__(
        ...
        self.aliked = ALIKED(
            pretrained=True, model_name="aliked-n32", detection_threshold=0.01,
        )
...
class TrackerKLT:
    ...
    def track_image(self, cur_img_dist, flow_back=True, max_cnt=256, min_dist=30):
        global global_id_counter
...
class TrackerALIKED:
    ...
    def should_reset(self, kpts, visibility):
        ...
        # wxy = wx * wy / (self.width * self.height)
        # cond_wxy = wxy < 0.7
        if self.last_reset >= 10 and np.sum(visibility) < self.n_tracks * 0.1:
            print(
                "#### Resetting model with new query points due to low visibility... ####"
            )
            return True

        elif np.sum(visibility) < 10:
            print(
                "#### Resetting model with new query points due to very low visibility... ####"
            )
            return True
...
class TrackerRaw:
    def __init__(self, *args, **kwargs):
        mode = os.environ.get("TRACKER_MODE", "tapnext_klt_1024").lower()
        if mode == "klt":
            self.tracker1 = None
            self.tracker2 = None
            ...
            self.tracker2 = TrackerKLT()
            self.tracker3 = None
        elif mode == "tapnext_klt_1024":
            self.tracker1 = TrackerTAPNext(
                onnx_path="/datasets/tapnext_1024.onnx",
                engine_path="/datasets/tapnext_1024_fp16.engine",
                n_tracks=1024,
                outlier_elimination=False,
                other_reset=True,
                reset_every=100,
