...
try:
    ...
    TAPNextTRT = None
from onnx_runner_test import TAPNextONNX
...
class TrackerTAPNext:
    def __init__(
        ...
        self.aliked = ALIKED(pretrained=True, detection_threshold=0.005)
    ...
    def track_image(self, frame_dist, max_cnt=None):
        ...
        if self.is_first_frame:
            ...
            if self.other_reset:
                ...
                # cond_wxy = wxy < 0.7
                if self.last_reset >= 2 and np.sum(self.is_valid) < self.n_tracks * 0.5:
                    print(
...
class TrackerRaw:
    def __init__(self, *args, **kwargs):
        mode = os.environ.get("TRACKER_MODE", "tapnext_klt_256").lower()
        if mode == "klt":
            self.tracker1 = None
            self.tracker2 = None
...
def outlier_elimination(kpts_1, kpts_2):
    ...
    _, mask = cv2.findFundamentalMat(kpts_1, kpts_2, cv2.FM_RANSAC, 3.0, 0.99)
    if mask is None:
...
@nb.njit
def simple_nms(keypoints, responses, min_dist=30.0, max_num=-1):
    N = keypoints.shape[0]
...
def main():
    ...
    tracker = TrackerTAPNext(
        onnx_path="/home/tdemirdal/Others/tapnext/tapnext_128.onnx",
        engine_path="/home/tdemirdal/Others/tapnext/tapnext_128.engine",
        n_tracks=128,
        reset_every=100,
        other_reset=True,
        outlier_elimination=False,
