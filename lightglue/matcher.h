...
class Matcher {
 public:
  // Construct with fixed network input size (width, height) and a sanity cap on keypoints.
  Matcher(const std::string& model_path, int width, int height, int n1, int n2);
  ~Matcher();

  // image1/image2: ORIGINAL images (any size, BGR or GRAY).
  // pts1/pts2: keypoints in ORIGINAL image coordinates (float); will be scaled internally.
  // Output: id1_to_2 with -1 for unmatched, length == pts1.size().
 ...
 private:
  ...
  Ort::Value makePtsTensor(const std::vector<int64_t>& flat_xy) const;  // int64 [N, 2]

  // Convert original points -> scaled (network frame) int64 XY (numpy-like rounding).
